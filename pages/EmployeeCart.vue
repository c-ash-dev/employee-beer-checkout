...
export default {
  ...
  computed: {
    // can get rid of this computed
    employeeId() {
      return this.$store.getters['autentication/userIdentification']
    },
    employeeName() {
      return this.$store.getters['employees/getEmployeeName'];
    },
    pointTotal() {
  ...
  methods: {
    checkOut() {
      this.$store.dispatch('employees/checkoutBeer', {
        points: this.pointTotal,
        // employeeId: this.employeeId - get rid of this
      });
